...
#import "SEEDPickeYMDHMSManager.h"
#import "SEEDPickerSectionItem.h"
#import "SEEDPickeDateModel.h"

/** yyyy-m-d hh:mm:ss*/
@implementation SEEDPickeYMDHMSManager
...
- (NSMutableArray <SEEDPickerSectionItem *>*)creatDateArrayWithIszh:(BOOL)isZh
    ...
    SEEDPickerSectionItem *secondItem = [SEEDPickerSectionItem new];
    secondItem.config = config;
    [secondItem.dataArray addObjectsFromArray:config.secondArray];
    [dataSource addObject:secondItem];
    ...
    [dateSource enumerateObjectsUsingBlock:^(SEEDPickerSectionItem * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        switch (idx) {
            ...
            case 4:{
                ...
                break;
            case 5:{
                NSInteger row = [obj.dataArray indexOfObject:config.second];
                if (row < obj.dataArray.count) {
                    [pickerView selectRow:row inComponent:5 animated:NO];
                    subConfig.second = config.second;
                }
            }
                break;
        }
...
- (void)selectFirstLineValueWith:(NSMutableArray *)dateSource {
    ...
    SEEDPickerSectionItem *itemMinute = [dateSource objectAtIndex:4];
    NSString *minute = [itemMinute.dataArray objectAtIndex:0];
    sourceConfig.minute = minute;
     
    SEEDPickerSectionItem *itemSecond = [dateSource objectAtIndex:5];
    NSString *second = [itemSecond.dataArray objectAtIndex:0];
    sourceConfig.second = second;
